  ...
  const getAvailableSlots = async () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (today.getDate() === currentDate.getDate()) {
        let startHour = currentDate.getHours() > 10 ? currentDate.getHours() + 1 : 10
        let startMin = currentDate.getMinutes() > 30 ? 30 : 0
        if (startHour >= 21) {
          startHour = 21
          startMin = 0
        }
        currentDate.setHours(startHour)
        currentDate.setMinutes(startMin)
      } else {
      ...
      setDocSlots(prev => {
        ...
        return next.filter(day => day.length > 0)
      })
    }
  }
      ...
      <div className='flex flex-col sm:flex-row gap-4'>
        ...
        <div className='flex-1 border border-gray-400 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0'>
          ...
          <p className='flex items-center gap-2 text-2xl font-medium text-gray-900'>
            {docInfo?.name}
            <img className='w-5' src={assets_frontend.verified_icon} alt="" />
          </p>
          ...
          <div>
            <p className='flex items-center gap-1 text-sm font-medium text-gray-900 mt-3'>
              About <img src={assets_frontend.info_icon} alt="" /></p>
